...
// ---------- Suggestions (incl. spiritual/moral) ----------
const SUGGESTIONS = {
  success: [
    "I ship the draft","I finish the proposal","I complete one hard thing","I protect two deep-work blocks",
    "I book the key meeting","I have one meaningful conversation","I close a lingering loop","I publish the update",
    "I make the first call","I delegate one task","I clear my inbox once","I move the top goal forward",
    "I fix the biggest blocker","I capture tomorrow’s plan","I review metrics and decide","I practice for 20 minutes",
    "I read 10 pages with notes","I record a 2-min Loom update","I run a 10-min retro","I confirm a decision with stakeholders",
    "I tidy my workspace for 10 min","I end on time","I help one person","I refine the roadmap in 15m",
    "I reduce scope once","I document one process","I pair with a teammate","I remove one dependency",
    "I sketch three options","I seek God’s will","I begin with prayer","I keep my word with integrity",
    "I forgive quickly","I serve someone quietly","I choose gratitude","I speak truth with grace","I keep sabbath-like margins",
  ],
  need: [
    "turn off notifications","put phone in another room","block 2×45m focus","define the next smallest step",
    "outline first, polish later","ask for feedback early","schedule a 15-min check-in","set a 25-min timer and start",
    "prep tools before starting","bundle shallow tasks (20m)","say no to one non-essential","hydrate and stretch (2m)",
    "draft three email templates","clarify the ‘done’ definition","move or shorten a meeting","clear the desk surface",
    "open the doc and write one sentence","commit to a 3-bullet outcome","close extra tabs","use headphones for immersion",
    "break it into a checklist","time-box the work on calendar","choose ambience and begin","tackle the hardest 10 minutes first",
    "mark blockers and ask early","group similar tasks","pin the focus window","set status to ‘heads-down’",
    "pray for 5 minutes","read a psalm","practice 3 minutes of silence","write one gratitude note",
    "reach out to encourage someone","confess and reset","step outside for a mindful walk","pause before I answer",
  ],
  become: [
    "present","calm and decisive","curious","generous","courageous","patient","kind and direct","lighthearted",
    "focused","resourceful","grateful","teachable","disciplined","humble","optimistic","resilient","attentive",
    "bold","steady","supportive","joyful","wise","hopeful","consistent","clear-minded","unhurried","encouraging",
    "prayerful","faithful","gentle and firm","Christlike",
  ],
};
...
export default function Page(){
  ...
  const [step, setStep] = React.useState(0); // 0 entry, 1 final, 2 review
  ...
  React.useEffect(()=>{ if(existingToday){ const last = latestForDate(entries, todayIso); setSuccess(last?.success||""); setNeed(last?.need||""); setBecome(last?.become||""); setStep(1);} },[]); // eslint-disable-line

  // seeded suggestions per day + stateful shuffle
  ...
  const [suggestMap, setSuggestMap] = React.useState({
    success: pick3(SUGGESTIONS.success, baseRng),
    need: pick3(SUGGESTIONS.need, baseRng),
    become: pick3(SUGGESTIONS.become, baseRng),
  });
            ...
            {step===0 && (
              <motion.section key="entry" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:0.2}} className="space-y-8">
                ...
                <div className="space-y-6 text-lg md:text-xl leading-relaxed">
                  <IntentRow id="success" labelPrefix="Today will be successful if" value={success} setValue={setSuccess} focused={focusIdx===0} locked={locked.success} onAdvance={()=>advanceFrom('success')} openFor={openFor} setOpenFor={setOpenFor} suggestions={suggestMap.success} onShuffle={()=>shuffle('success')} />
                  <IntentRow id="need" labelPrefix="I need to" value={need} setValue={setNeed} focused={focusIdx===1} locked={locked.need} onAdvance={()=>advanceFrom('need')} openFor={openFor} setOpenFor={setOpenFor} suggestions={suggestMap.need} onShuffle={()=>shuffle('need')} />
                  <IntentRow id="become" labelPrefix="I want to be" value={become} setValue={setBecome} focused={focusIdx===2} locked={locked.become} onAdvance={()=>advanceFrom('become')} openFor={openFor} setOpenFor={setOpenFor} suggestions={suggestMap.become} onShuffle={()=>shuffle('become')} />
                </div>
              ...
              <motion.section key="review" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:0.2}} className="space-y-8">
                ...
                <div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    {days.map((d)=> {
                      const level = ratings[d.date]==='high'?3: ratings[d.date]==='mid'?2: ratings[d.date]==='low'?1: 0;
                      const bg = level===3? ACCENT : level===2? hexToRGBA(ACCENT,0.6) : level===1? hexToRGBA(ACCENT,0.25) : "#e5e7eb";
                      return <div key={d.date} className="h-2.5 w-2.5 rounded-full" style={{ background:bg }} title={d.displayDate} aria-label={d.displayDate} />;
                    })}
                  </div>
